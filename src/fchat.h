 ...
 * Все данные передаются UDP пакетами (datagrams)
 * Каждый пакет разбит на блоки разделённые символом #1 и идентификатором блока.
 ...
 * Блок подтверждение сообщения особенный: он пустой. Но если он есть тогда требуется подтверждение сообщения.
 * Ограничения:
 * * Чат комната возможна только одна. В стандартном ФЧат приватные комнаты тоже непонятно как работают.
 ...
 * * Иконки статусов отсутствуют. Их нужно реализовывать через xStatuses а они появятся в Pidgin не скоро (если вообще появятся).
...
#define FCHAT_MSG_ID_BLOCK           9   /**< Номер сообщения                              */
#define FCHAT_MSG_CONFIRMATION_BLOCK 10  /**< Требование подтверждения о доставке (пустой) */
#define FCHAT_MSG_BLOCK              255 /**< Текст сообщения                              */
...
#define FCHAT_PONG_CMD            'Y' /**< Связь ОК                                */
...
typedef struct {
	...
	gchar *msg_id;           /**< Номер сообщения                              */
	gchar *msg_confirmation; /**< Требование подтверждения о доставке (пустой) */
	gchar *msg;              /**< Текст сообщения. Обязательно последний!      */
} FChatPacketBlocks;
